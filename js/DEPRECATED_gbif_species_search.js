...
This is function is deprecated but left in place for future reference.
*/
export async function commonSearch(dataConfig, searchTerm) {
  let reqHost = gbifApi;
  let reqRoute = "/species/search";
  let reqQuery = `?q=${searchTerm}`;
  let reqFilter = `&${dataConfig.speciesFilter}`; //can be a species datasetKey, a list of taxa, or something else
  let reqField = `&qField=VERNACULAR`;
  let url = reqHost+reqRoute+reqQuery+reqFilter+reqField;
  let enc = encodeURI(url);
